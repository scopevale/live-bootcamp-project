...
mod tests {
    ...
    fn email_missing_domain_is_rejected() {
        let email = Secret::new("user@".to_string());
        assert!(Email::parse(email).is_err());
    }
    #[test]
    fn email_invalid_character_in_username_is_rejected() {
        let email = Secret::new("bad>user@domain.com".to_string());
        assert!(Email::parse(email).is_err());
    }
    #[test]
    fn email_invalid_character_in_domain_is_rejected() {
        let email = Secret::new("gooduser@bad&domain.com".to_string());
        assert!(Email::parse(email).is_err());
    }
    ...
    fn valid_emails_are_parsed_successfully(valid_email: ValidEmailFixture) -> bool {
        Email::parse(valid_email.0.into()).is_ok()
    }
}
